...
func Test_createHandsUpCommandsForBetweenNowAndTomorrow_今日明日のイベントが取得できる(t *testing.T) {
	jst, _ := time.LoadLocation("Asia/Tokyo")
	result := createHandsUpCommandsForBetweenNowAndTomorrow(sampleSQInfo, time.Date(2023, 10, 27, 0, 0, 0, 0, jst))
	expected := []string{
...
func Test_createHandsUpCommandsForBetweenNowAndTomorrow_今日明日のイベントがない場合_空のsliceを返却する(t *testing.T) {
	jst, _ := time.LoadLocation("Asia/Tokyo")
	tests := []struct {
		name string
		now time.Time
		...
		{
			name: "nowが2023-10-22 23:59:59の場合",
			now: time.Date(2023, 10, 22, 23, 59, 59, 0, jst),
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := createHandsUpCommandsForBetweenNowAndTomorrow(sampleSQInfo, tt.now)
			expected := []string{}
			if len(result) != 0 {
				t.Errorf("len(result) = %d, want %d", len(result), len(expected))
			}
